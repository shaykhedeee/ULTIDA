...
export const SceneV1Schema = z.object({
  ...
  modules: z.array(z.object({ id: Id, roomId: Id, family: z.string(), widthMm: z.number().positive(), depthMm: z.number().positive(), heightMm: z.number().positive(), position: PointMm.extend({ zMm: z.number().nonnegative().default(0) }), rotationDeg: z.number().finite(), anchor: z.enum(['floor','wall','ceiling','free']), materialId: Id.optional(), glbUrl: z.string().refine((value) => value.startsWith('/') || /^https:\/\//.test(value), 'GLB URL must be an absolute HTTPS URL or an app asset path.').optional(), confidence: Confidence })),
