import { Download, ImagePlus, ShieldCheck, Sparkles } from 'lucide-react';
import { useState } from 'react';
import './cnc-pattern-studio.css';

type Pattern = 'diamond' | 'arch' | 'circle';
const patternNames: Record<Pattern, string> = { diamond: 'Diamond jaali', arch: 'Arch lattice', circle: 'Circle lattice' };
...
export function CncPatternStudio() {
  ...
  const valid = width >= 100 && height >= 100 && spacing >= 30 && spacing <= Math.min(width, height);
  function download() {
    if (!valid) return;
  ...
  return <div className="cnc-pattern-studio">
    ...
    <section className="cnc-panel"><h2>2. Choose a template</h2><div className="cnc-patterns">{(Object.keys(patternNames) as Pattern[]).map((value) => <button key={value} className={pattern === value ? 'active' : ''} onClick={() => setPattern(value)}><Sparkles size={16}/><strong>{patternNames[value]}</strong><small>{value === 'diamond' ? 'Classic linear CNC panel' : value === 'arch' ? 'Soft European repeat' : 'Ventilated round lattice'}</small></button>)}</div><div className="cnc-inputs"><label>Panel width (mm)<input type="number" min={100} value={width} onChange={(event) => setWidth(Number(event.target.value))}/></label><label>Panel height (mm)<input type="number" min={100} value={height} onChange={(event) => setHeight(Number(event.target.value))}/></label><label>Pattern pitch (mm)<input type="number" min={30} value={spacing} onChange={(event) => setSpacing(Number(event.target.value))}/></label></div>{!valid && <p className="cnc-error">Use a panel of at least 100 mm and a pitch between 30 mm and the smallest panel dimension.</p>}<p className="cnc-note">For production: confirm tool diameter, bridge thickness, material thickness and machine origin with your CNC operator before cutting.</p></section></div>
  </div>;
}
